...
public class Order {
    ...
    // BOM: FetchType.LAZY para coleções com @BatchSize
    @OneToMany(mappedBy = "order", fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    @BatchSize(size = 50)
    private List<OrderItem> orderItems;

    // BOM: @Lob com FetchType.LAZY para blobs
    @Lob
    @Basic(fetch = FetchType.LAZY)
    @Column(name = "invoice_pdf")
    private byte[] invoicePdf;

    // MÁ PRÁTICA: Comentado - FetchType.EAGER para @ManyToOne
    ...
    // MÁ PRÁTICA: Comentado - Sem índices em colunas pesquisadas
    // @Column(name = "order_date", nullable = false)
    // private LocalDateTime orderDate = LocalDateTime.now();
